...
- Scoped MCP tool brokering across stdio, streamable HTTP, and agentgateway with exact profile/role/Task Contract authorization, all-or-nothing discovery, durable approval, at-most-one dispatch, uncertain-effect disposition, bounded untrusted results, dynamic readiness capabilities, and restart-safe recovery (#9).
...
- Keep MCP credentials, endpoints, headers, stderr, native SDK values, raw arguments, and native results out of public evidence; reject schema/reference/header/authority widening, suppress automatic retry after uncertain effects, and package no private broker-store declarations.
...
- Worker/fallback and agent-loop execution remain pending Issue #10, independent review execution proof remains pending Issue #11, final reconciliation/evidence remains pending Issue #12, full runtime hardening remains pending Issue #13, and protected live-provider and live-credential smoke remains pending Issue #15. The protected live-credential test is not included in ordinary CI.
