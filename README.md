...
`planModelSelection()` uses fixed ASCII deterministic ordering over policy preference, catalog priority, worst-case cost, latency, and stable entry identity. Reordered and rehashed catalog or live-route arrays preserve semantic selection while the plan and next-state hashes rebind the exact authoritative bytes. For security, architecture, or irreversible risk, independent review planning requires a reviewer whose every provider/model pair differs from every accepted primary and fallback route. This is a planning guarantee only; Issue #11 remains pending for independent worker/reviewer execution proof.

Money is nonnegative integer microusd. Each uncached-input, cached-input, ordinary-output, and reasoning-output component is multiplied with `bigint`, divided by one million, and rounded upward independently. The primary, every planned fallback, and the reviewer are reserved atomically against input, output, cost, duration, and turn limits. Actual settlement releases unused allocations and preserves overages; missing trusted usage after a possible provider effect makes the budget unknown until Issue #12 remains pending for authoritative usage reconciliation.

Circuit state is explicit and hash-linked. Only timeout, transient-provider, and gateway-unavailable outcomes advance it or authorize an explicit fallback already present in the plan. `recordRoutingOutcome()` returns a hashed outcome witness; `nextModelFallback()` consumes that witness without invoking transport or retrying. Settlement proves every intermediate circuit-only revision with a bounded `circuit_state_chain`. Governed override narrowing can choose only an already eligible catalog entry and cannot weaken capability, latency, review, fallback, circuit, or budget policy.
