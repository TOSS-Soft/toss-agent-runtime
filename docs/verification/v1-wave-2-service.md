...
`cdf05aa09e863cc199e4b082e30e6ec9e1f3c5c8` on 2026-08-20. That commit was
...
| ------------------ | ------------------------------------------------ |
| Date               | `2026-08-20`                                     |
| Publishable commit | `cdf05aa09e863cc199e4b082e30e6ec9e1f3c5c8`       |
| Node.js            | `v22.23.1`                                       |
| npm                | `11.18.0`                                        |
...
| Vitest                      | 19 files passed; 467 tests passed and 1 host-conditional test skipped (468 total).                                                                                                                                                                                   |
...
| Packed files     | 178                                                                                               |
| Packed size      | 116,846 bytes                                                                                     |
| Unpacked size    | 624,658 bytes                                                                                     |
| npm SHA-1 shasum | `229fa74cf6c40c47e72ce8631f7dc2a10585001d`                                                        |
| npm integrity    | `sha512-qCvO+ngpOEm640Zn0HTkxUqMprZoejZdpTK07tcUjqCvNflV6HsQZz8UFtnVTVGZMTTrwhgdUutU/3u0ar9Djg==` |
| SHA-256          | `38a3cd8b3a5217f2843676e0869a5f7035b113168ca2b4baf8d262ae0b839c2e`                                |
...
Five older task-named test roots that predated this acceptance were inspected
and moved together to a collision-safe Trash directory, rather than deleted,
before the empty final probe. No launchd/systemd user manager was mutated.
...
| Crash loops produce bounded backoff and `doctor` provides actionable diagnosis.                     | Direct restart/status policy verified               | Direct SIGKILL/restart smoke proves stale-runtime reclamation. Definitions encode bounded restart policy; Linux terminal `Result=start-limit-hit` now deterministically yields `backoff:true` and a doctor `FAIL`. Actual native-manager crash-loop observation remains platform-integration pending.                                           |
