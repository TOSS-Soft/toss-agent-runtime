...
describe("skill runtime contracts", () => {
  ...
  it("rejects a snapshot with duplicate or non-ASCII ordered resources", () => {
    ...
    const unordered = { ...fixture, resources: [{ ...fixture.resources[0], path: "z.md" }, { ...fixture.resources[0], path: "A.md" }] };
  ...
  it("rejects uppercase hashes and bad document hashes", () => {
    ...
    expect(parseSkillDescriptor(canonicalJson({ ...fixture, package_hash: `sha256:${"A".repeat(64)}` }))).toMatchObject({ ok: false });
    expect(parseSkillDescriptor(canonicalJson({ ...fixture, document_hash: `sha256:${"f".repeat(64)}` }))).toMatchObject({ ok: false });
  });

  it("rejects phases with status/output combinations that cannot occur", () => {
    const fixture = validSuperpowersPhase();
    expect(parseSuperpowersPhase(canonicalJson({ ...fixture, status: "STARTED", output_hash: fixture.output_hash }))).toMatchObject({ ok: false });
    expect(parseSuperpowersPhase(canonicalJson({ ...fixture, status: "COMPLETED", output_hash: null }))).toMatchObject({ ok: false });
  });
  ...
  it("rejects evidence with duplicate resource and phase hashes", () => {
    ...
    expect(parseSkillExecutionEvidence(canonicalJson({ ...fixture, resource_hashes: [fixture.resource_hashes[0], fixture.resource_hashes[0]] }))).toMatchObject({ ok: false });
    expect(parseSkillExecutionEvidence(canonicalJson({ ...fixture, phases: [fixture.phases[0], fixture.phases[0]] }))).toMatchObject({ ok: false });
  });
